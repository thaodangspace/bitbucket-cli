...
var capabilityRegistry = map[string]Capability{
	...
	"webhook.write":           {Operation: "webhook.write", AuthKinds: []string{"api", "access", "oauth"}, Scopes: []string{"read:webhook:bitbucket", "write:webhook:bitbucket"}, Available: true, Note: "Webhook creation and updates require both the dedicated webhook read and write scopes; subscribed event types may add their own requirements."},
	...
	"webhook.apply":           {Operation: "webhook.apply", AuthKinds: []string{"api", "access", "oauth"}, Scopes: []string{"read:webhook:bitbucket", "write:webhook:bitbucket", "delete:webhook:bitbucket"}, Available: true, Note: "Apply reads and writes webhooks; --prune additionally requires the delete scope."},
	"ssh-key.read":            {Operation: "ssh-key.read", AuthKinds: []string{"api", "access", "oauth"}, Scopes: []string{"read:ssh-key:bitbucket"}, Available: true, Note: "Account SSH-key reads require the SSH-key read scope; default authenticated-user resolution additionally requires read:user (explicit --user avoids that lookup)."},
	"ssh-key.write":           {Operation: "ssh-key.write", AuthKinds: []string{"api", "access", "oauth"}, Scopes: []string{"read:ssh-key:bitbucket", "write:ssh-key:bitbucket", "read:user:bitbucket"}, Available: true, Note: "Account SSH-key creation and label updates require SSH-key read/write scopes; default authenticated-user resolution also requires read:user."},
	"ssh-key.delete":          {Operation: "ssh-key.delete", AuthKinds: []string{"api", "access", "oauth"}, Scopes: []string{"delete:ssh-key:bitbucket", "read:user:bitbucket"}, Available: true, Note: "Account SSH-key deletion requires the SSH-key delete scope; default authenticated-user resolution also requires read:user."},
